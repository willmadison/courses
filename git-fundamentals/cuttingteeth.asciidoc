...
TIP: In order to write a commit message using this vim-like editor type the letter `i` to enter "insert mode" (basically the mode most commonly used text editors are in by default). Type your message, then hit the `esc` key to go back into "command mode". Once you're happy with the message type in `:wq` (which means write (save) and quit) to exit the editor.
